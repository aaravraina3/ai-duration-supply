...
- [ ] 1.1 Pull and cache the buyback operations table
- [ ] 1.2 Map maturity buckets to representative tenors, convert par accepted to
      ...
      the assumption
- [ ] 1.3 Build a daily long-end buyback duration series, 2024 onward
- [ ] 1.4 Add to the term premium regression next to AI supply. Report both
      coefficients in the same table
- [ ] 1.5 Local projections on buyback operations, h = 0..60, same spec as the
      issuance side, so the two impulse responses are directly comparable
- [ ] 1.6 Randomization test on the buyback coefficient, same 1000-draw protocol
...
- [ ] 2.1 Build the 5y5y forward from the GSW curve. It is further from the
      hedging point than the 10Y and less contaminated
- [ ] 2.2 Re-run the announcement local projection with 5y5y as the dependent
      variable
...
- [ ] 3.1 Pull FRED `IRLTLT01JPM156N` (JGB 10Y, monthly) and
      `IR3TIB01JPM156N` (JPY 3m, monthly)
- [ ] 3.2 Build the hedged pickup: `UST10Y − (USD 3m − JPY 3m) − JGB10Y`,
      monthly, using DGS10 and DTB3 month-end
- [ ] 3.3 Pull TIC data on Japanese Treasury holdings for the actual flow series
- [ ] 3.4 Regress monthly ΔTP10 on the hedged pickup with controls. Report that
      n ≈ 32 and do not over-claim
- [ ] 3.5 Compare scale directly: Japanese net selling in $bn of 10-year
...
- [ ] 5.1 **H4 needs redoing.** The Treasury-supply null was run on a period
...
- [ ] 5.2 Extend the issuance sample. September deals exist (Amazon priced a GBP
      ...
      EDGAR and refresh
- [ ] 5.3 Mark the 2025 Q4 macro calendar unreliable. The Oct 1 to Nov 12 2025
...
- [ ] 6.1 Keep `03_FINDINGS.md` as the only place a number is stated as
      established. Everything else links to it
- [ ] 6.2 Any new specification gets logged in `02_HYPOTHESES.md` with its
      falsification criterion **before** it runs, given how much unlogged
      specification search already happened
- [ ] 6.3 Every claim in a deliverable carries its inference method. "NW t" alone
